...
export default {
  ...
  methods: {
    ...
    signUp() {
      ...
      // this.$store.commit("addUsers", userObj); //this.$store.commit('mutation 이름', 인자(payload))
      //  EventBus.$emit("signUp", userObj);
      // this.addUsers(userObj); // store.js의 payload 위치로 감
      // this.$store.dispatch("addUsers", userObj); // action을 불러옴
